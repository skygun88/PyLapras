...
class TSEAgent(LaprasAgent):
    def __init__(self, 
        ...
        self.mask = np.array([1,1,1,1,1,1])

        N_INS = [N_IN_ACT, N_IN_CLO, N_IN_AGE, N_IN_GEN]
        n_in_vector = 36 + sum([N_INS[goal] for goal in self.input_config])
        s_dim = ((1, 1, 84, 84), n_in_vector) if self.gray else ((1, 3, 84, 84), n_in_vector)
        a_dim = 6
        

        self.drl_net = Net(s_dim, a_dim, isMgCl=MgCl, isinvDM=invDM, isMeta=isMeta)   
        # drl_fpath = '/SSD4TB/skygun/robot_code/RobotTSE/weight/runs/20231605_195533/simple_navigation_weight_2000.pth'
        drl_fpath = '/SSD4TB/skygun/robot_code/RobotTSE/weight/runs/20232405_131056/simple_navigation_weight_1400.pth'
        
        self.drl_net.epsilon = 0.01
        self.drl_net.load_state_dict(torch.load(drl_fpath, map_location='cpu'))
        self.drl_net = self.drl_net.to(self.device)
        self.progress = np.array([1, 1, 1, 1])
        # self.similar_calibration = True
        self.similar_calibration = False
    ...
    def load_train_dataset(self):
        self.train_data_df: pd.DataFrame = pd.read_excel(io='/SSD4TB/skygun/robot_code/RobotTSE/Utils/dataset_with_face.xlsx')
        self.train_scenarios: np.ndarray = self.train_data_df['dataset'].unique()
        self.train_secnario_len: int = len(self.train_scenarios)
        self.train_data_df['full_img'] = self.train_data_df.apply(lambda x: cv2.imread(x['full_img_path']), axis=1)
        self.train_data_df['face_img'] = self.train_data_df.apply(lambda x: cv2.imread(x['face_img_path']) if pd.notna(x['face_img_path']) else None, axis=1)
    
    def find_similar_scene(self, curr_img, curr_rstate):
        SIM_RESULTS = np.zeros((self.train_secnario_len))
        IMGS = []
        for idx, scenario in enumerate(self.train_scenarios):
            datas = self.train_data_df.loc[(self.train_data_df.dataset==scenario) & (self.train_data_df.robot_pos==curr_rstate), :]   
            data_len = len(datas)
            start_idx = random.randint(0, data_len-1)
            frames = datas[start_idx:start_idx+1][['full_img']]
            full_img = frames['full_img'].to_numpy()[-1]

            ssim_result = ssim(full_img, curr_img, win_size=3)
            SIM_RESULTS[idx] = ssim_result
            IMGS.append(full_img)
        
        best_index = np.argmax(SIM_RESULTS)

        # print(best_index, self.train_scenarios[best_index])
        similar_scene = IMGS[best_index]
        return similar_scene, self.train_scenarios[best_index]
    ...
    def determine_action(self):
        ...
        if self.similar_calibration:
            curr_img = self.curr_img.copy()

            ori_h, ori_w, _ = curr_img.shape
            curr_img = curr_img[:, (ori_w//2)-(ori_h//2):(ori_w//2)+(ori_h//2), :]
            curr_img = cv2.resize(curr_img, dsize=(84, 84), interpolation=cv2.INTER_AREA)
            similar_scene, scenario = self.find_similar_scene(curr_img, self.curr_rstate)
            self.curr_img = similar_scene
            print(f'similar scene: {scenario}')

        observation = (self.curr_pose, *self.curr_confs, self.curr_img)
        s_i, s_v = preprocess(observation, self.input_config, self.gray)
        hs = torch.zeros(1, 256).to(self.device) if self.reset else self.h_s.detach()
        cs = torch.zeros(1, 256).to(self.device) if self.reset else self.c_s.detach()
        self.reset = False
        ...
        if self.is_masking:
            self.visited[self.curr_rstate] += 1
            self.rstate_queue.append(self.curr_rstate)
            possible_rstate = self.get_possible_rstate(self.curr_rstate)[1:5]
            possible_actions = [1 if self.visited[rs] < 1 else 0 for rs in possible_rstate]
            self.mask = np.array([1, *possible_actions, 1])
            # if (len(self.rstate_queue) == 3) and (list(self.rstate_queue).count(self.curr_rstate) == 3):
            #     self.mask[0] = 0


        if self.curr_goal == None:
            a, self.h_s, self.c_s, g = self.drl_net.choose_action(
                v_wrap(s_i[None, :], device=self.device), 
                v_wrap(s_v[None, :], device=self.device),
                hs,
                cs,
                v_wrap(self.progress[None, :], device=self.device),
                v_wrap(self.mask[None, :], device=self.device),
            )
            self.curr_goal = g
            
        else:
            a, self.h_s, self.c_s = self.drl_net.choose_action_with_goal(
                v_wrap(s_i[None, :], device=self.device), 
                v_wrap(s_v[None, :], device=self.device),
                hs,
                cs,
        ...
        if a == STAY:
            ...
            if is_detected: 
                prediceted = self.curr_preds[self.curr_goal]
                self.determined[self.curr_goal] = prediceted
