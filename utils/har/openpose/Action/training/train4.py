...
# load data
raw_data = pd.read_csv('web_har_dataset2.csv', header=0)

dataset = raw_data.values
# X = dataset[:2264+2453+2899+2435+1500, 0:36].astype(float)
X = dataset[:, 0:36].astype(float)
Y = dataset[:, 36]

### Custom ###
# encoder_Y = [0]*2264 + [1]*2453 + [2]*2899 + [3]*2435 + [4]*2174
# data_len = 1307
encoder_Y = [0]*1319 + [1]*1319 + [2]*1319

# one hot 编码
dummy_Y = np_utils.to_categorical(encoder_Y)

print(X.shape, Y.shape, dummy_Y.shape)

# train test split
X_train, X_test, Y_train, Y_test = train_test_split(X, dummy_Y, test_size=0.2, random_state=9)
...
model.summary()
his.loss_plot('epoch')
# model.save('framewise_recognition2.h5')
model.save('web_lounge2.h5')
